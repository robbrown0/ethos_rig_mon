...
hashavgalert = 95 # Set the hashaverage variable to the rate in MH/s that you want the rig to reboot at.  Depends on the rigs total MH/s.
...
errorstatusmessage = "NOT HASHING"
...
#
# *** NOTE *** You MUST edit lines 55, 56 (these are your CRONTAB entries) and lines 64 and 72!!  You do NOT use the < and > around the tokens.
#
...
#
# The script uses the Pushover notification service to notify you of reboots or when a card or the rig are not hashing as expected.
...
@reboot curl -s -F "token=<app_token>" -F "user=<user_token> " -F "title=RIG - "$(hostname)" - IS NOW ONLINE" -F "message=RIG - "$(hostname)" rebooted and is now online." https://api.pushover.net/1/messages.json
*/2 * * * *   /usr/bin/python /home/ethos/ethos_rig_mon.py -u <user_token> -a <app_token> >> /home/ethos/ethos_rig_mon.log
...
if not os.path.exists("/home/ethos/ethos_rig_mon.log"):
    os.system("python ethos_rig_mon.py -u <user_token> -a <app_token> >> /home/ethos/ethos_rig_mon.log") 
    exit()
...
# based on ping results, it either writes an offline log entry and exits, of if the result was online, the script continues.
if is_up <> True:
...
varCardHash = 0
varCardLog = ""
f = '/var/run/ethos/miner_hashes.file'
...
if err:
    ...
    for varCardHash in (varPerCardHash):
        ...
        if not hash or hash < hashalert:
            ...
            #print "/tmp/ethos_rig_mon_gpu_"+varCardLog+".txt"
            if os.path.exists("/tmp/ethos_rig_mon_gpu_"+varCardLog+".txt"):
                os.remove("/tmp/ethos_rig_mon_gpu_"+varCardLog+".txt")
...
# Check RIG average hash, if is non-existant of less than 115 by default - defined in the hashavgalert variable, a temp file is written, and if on the second run, the file still exists, the rig is rebooted.
